(function() {
  ...
  Alchemy = (function() {
    ...
    Alchemy.prototype.getInst = function(svg) {
      ...
      instNumber = parseInt(d3.select(svg).attr("alchInst"));
        //Vipul Fix alchemyJS for a.conf error
        instNumber+=1
  ...
  Alchemy.prototype.defaults = {
    ...
    graphWidth: function() {
        //Vipul width fix
      return d3.select(this.divSelector).node().parentElement.clientWidth - 30;
    },
    graphHeight: function() {
      if (d3.select(this.divSelector).node().parentElement.nodeName === "BODY") {
        return window.innerHeight;
      } else {
        return d3.select(this.divSelector).node().parentElement.clientHeight-30;
      }
    },
    alpha: 0.5,
