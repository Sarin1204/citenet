    ...
    '<input type="text"' +
     ...
     '<ul ng-show="dropdownVisible">' +
      '<li ng-repeat="item in dropdownItems"' +
          ...
          '>' +
        '<span ng-if="item.role_name">{{item.role_name}}</span>' +
        '<span ng-if="!item.role_name">{{item}}</span>' +
      '</li>' +
    '</ul>' +
  '</div>';
  ...
  return {
    ...
    link: function(scope, element) {
      ...
      scope.$watch('selectedItem', function(newValue, oldValue) {
        ...
        if (!angular.equals(newValue, oldValue)) {
          if (newValue) {
            // Update value in input field to match role_name of selected item
            if (typeof newValue === 'string') {
              scope.inputValue = newValue;
            }
            else {
              scope.inputValue = newValue.role_name;
            }
          }
          else {
